...
fn main() {
    ...
    // 1. 全銘柄コードを取得
    let all_codes = get_unique_codes();
    println!("全銘柄数: {}", all_codes.len());

    // 2. range 引数（例: "1000-3000"）がある場合のフィルタリング
    let target_codes: Vec<String> = if let Some(ref range_str) = args.range {
        let parts: Vec<&str> = range_str.split('-').collect();
        let start: u32 = parts[0].parse().unwrap_or(0);
        let end: u32 = parts[1].parse().unwrap_or(9999);

        all_codes.into_iter().filter(|code| {
            // "1301" や "4052.T" から数値部分だけを抽出して判定
    ...
    println!("🎯 今回の処理対象銘柄数: {}", target_codes.len());
    // ...
}

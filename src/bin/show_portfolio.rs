...
async fn main() -> Result<()> {
    ...
    // 2. 現在のポジション表示
    println!("\n【📈 現在保有中のポジション】");
    println!(" 銘柄コード | 銘柄名               | 購入日     | 購入単価 | 現在値  | 数量 | 評価損益 (最高値)");
    println!("-------------------------------------------------------------------------------------------------");
    ...
    while let Some(row) = rows.next()? {
        ...
        println!(" {:<10} | {:<20} | {:<10} | {:>8.1} | {:>7.1} | {:>4} | {:>+9.1}円 ({:>7.1})",
    ...
    // 3. 決済履歴の表示 (最新10件)
    println!("\n【📜 直近の決済履歴】");
    println!(" 銘柄コード | 銘柄名               | 購入日     | 決済日     | 購入単価 | 決済単価 | 数量 | 確定損益");
    println!("-------------------------------------------------------------------------------------------------");

    let mut stmt_hist = conn.prepare("SELECT code, name, entry_date, exit_date, entry_price, exit_price, qty, profit_loss FROM trade_history ORDER BY id DESC LIMIT 10")?;
    ...
    while let Some(row) = rows_hist.next()? {
        ...
        println!(" {:<10} | {:<20} | {:<10} | {:<10} | {:>8.1} | {:>8.1} | {:>4} | {:>+9.1}円",

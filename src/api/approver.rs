    ...
    pub async fn approve_trade(
        ...
        let prompt = format!(
            "あなたはプロのクオンツ・システムトレーダーです。\n\
             ...
             3. あなた自身が「リスク（risk_factor）」の項目に「固有の材料が確認できない」と記述する場合は、論理적整合性を保つため、スコアは必ず0.30以下、判定は必ずNO-GO、has_distinct_material は false としなければならない。0.40以上のスコアをつけることは自己矛盾であり、厳禁とする。\n\n\
             # 出力フォーマット (JSONのみ)\n\
             {{\n\
